...
> Python используется только дополнительными скриптами для первоначальной настройки Wi-Fi и снятия LAN-блокировки.
...
Другие ревизии ламп BroadLink пока не проверены на реальном оборудовании и не должны считаться официально поддерживаемыми.
...
Установите плагин через интерфейс Homebridge, найдя:
...
## Подготовка лампы для локального управления
...
Если лампа уже подключена к Wi-Fi и при проверке показывает `Locked: False`, этот раздел можно пропустить и перейти к [настройке Homebridge](#настройка-homebridge).

Скрипты из `scripts/` используют [`python-broadlink`](https://github.com/mjg59/python-broadlink) только на этапе первоначальной подготовки. После этого Homebridge их не использует.

### 1. Получите скрипты подготовки
...
### 2. Установите зависимости вспомогательных скриптов

Создайте временное Python-окружение:
...
### 3. Переведите лампу в режим настройки
...
### 4. Передайте лампе параметры Wi-Fi
...
### 6. Снимите блокировку локального управления
